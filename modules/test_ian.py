...
def lineups_explorations(hist_lineups_dict):
	...
	for date in hist_lineups_dict.keys():
		for contest,contest_dict in hist_lineups_dict[date].iteritems():
			points_list.append(contest_dict['points'])
	...
	large_contest_average=[contest_dict['points'] for date in hist_lineups_dict.keys() 
								for contest,contest_dict in hist_lineups_dict[date].iteritems() if contest_dict['player_universe_size']>70]
	print len(large_contest_average)
	print np.mean(large_contest_average)
	print np.median(large_contest_average)
	plot(player_universe_size,points_list)
	# plot(sum_confidence,points_list)
	return
